...
function quickView(productItem,price,discount_price,description,brand){
	if($(window).width() > 1025){
		var quickView= $(".quickview"),
                thumbList = quickView.find(".container"),
                clicked=false;
        //Get content
        // console.log(productItem);
        // quickViewGetContent(productHandle);
        $(".product-name-quick").text(productItem['name']);
        $(".product-price-quick").text(price);
        $(".product-price-old-quick").text(discount_price);
        $(".product-description-quick").text(description);
        $(".brand-product-quick").text(brand);
        $(".image-product-quick").attr('src',productItem['image']);

        $("#btn-addCart").on('click',function(){
            senDataAjax(productItem['id']);
            productItem='',price='',discount_price='',description='',brand='';
        });
