	...
	slider.slider({
		...
		slide: function(event, ui) {
			...
			if(ui.values[0]==0){
				...
				var uimin =parseInt(ui.values[0])-1;
				var uiminS =parseInt(ui.values[0])-1;
			}

			if(ui.values[1]>=maxx-1){
				var uimax = maxx;
				var uimaxS=maxx-1;
			}else{
				var uimax = parseInt(ui.values[1])+1;
				var uimaxS = parseInt(ui.values[1])+1;
			}


			$('#start span').text(DL_formatmoney(uiminS-1, "{{amount_no_decimals_with_comma_separator}}₫"));
			$('#stop span').text(DL_formatmoney(uimaxS+1, "{{amount_no_decimals_with_comma_separator}}₫"));
			$('#filter-value').attr('data-value','(price:product>='+uimin+')&&(price:product<'+uimax+')');
